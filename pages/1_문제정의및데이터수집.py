...
# 선생님 피드백 반영: 독립변수, 종속변수 및 모델 유형 최종 정의
...
with col_var1:
    ...
    st.markdown("""
    선생님 피드백을 반영하여, 우리가 사용할 Kaggle 데이터셋의 특성에 맞게 독립변수와 종속변수를 아래와 같이 정의합니다.
    ...
    * **🎯 종속변수 (Target Variable / 결과가 되는 예측 대상)**
        * **[회귀용]** `peopleLost` (플레이어들이 잃은 총 금액) -> 수치 예측
        * **[분류용]** `is_high_risk` (손실액과 참여 빈도를 조합해 자체 라벨링한 '중독 위험 등급': 0 또는 1) -> 범주 예측
    """)

with col_var2:
    st.subheader("🧠 최종 선택한 인공지능 모델 유형")
    st.markdown("""
    본 프로젝트는 청소년에게 다각적인 경각심을 주기 위해 **[회귀]와 [분류] 모델을 모두 결합한 하이브리드 형태**를 선택했습니다.
    
    1. **회귀(Regression) 모델 선택 이유**
        * **목표:** 독립변수(`money`, `ticks` 등)를 가지고 종속변수인 **'실제 손실 금액($)'**이라는 연속된 숫자를 예측합니다.
        * **이유:** 청소년들에게 "너 이 조건으로 베팅하면 구체적으로 **몇만 원 잃게 돼**"라는 정량적인 수치 경고를 주기 위함입니다.
        
    2. **분류(Classification) 모델 선택 이유**
        * **목표:** 유저의 행동 패턴 데이터를 분석하여 **'정상군(0) / 중독 위험군(1)'**이라는 명확한 그룹(범주)으로 나눕니다.
        * **이유:** 손실 금액을 넘어, 이 학생이 현재 치료나 상담이 필요한 **'위험 상태'인지를 판별**하여 학교 상담실 시스템과 연계하기 위함입니다.
    """)

st.success("📝 선생님 피드백 반영 완료: 독립변수/종속변수 매칭 및 회귀·분류 이원화 설계 구조 확정.")
