...
# 2. 데이터 고정 로드 및 캐싱 (성능 최적화)
@st.cache_data
def load_clean_data():
    # 업로드해주신 전처리 파일명과 정확히 일치시켜 불러옵니다.
...
# 파일 로드 실패 시 직관적인 가이드 화면 출력
if not file_loaded:
    st.error("📂 'onlineCasino_cleaned_v2.csv' 파일을 찾을 수 없습니다.")
    st.markdown("""
    **🛠️ 1초 해결 방법:**
    현재 이 코드가 실행 중인 `app.py` 파일과 **동일한 폴더(경로)** 안에 다운로드 받으신 **`onlineCasino_cleaned_v2.csv`** 파일을 넣어주세요! 
    이름이나 위치가 다르면 파이썬이 파일을 읽지 못합니다.
    """)
    st.stop()
...
target_var = st.sidebar.selectbox(
    "시각화 기준 종속변수 선택", 
    ["outpay", "peopleLost"],
    help="선택한 종속변수에 따라 아래 분포차트와 박스플롯이 실시간으로 변경됩니다."
)
...
with tab1:
    ...
    st.markdown("정제 완료된 수치형 변수들과 새로 생성한 파생변수들이 종속변수들과 어떠한 관계를 가지는지 분석합니다.")
