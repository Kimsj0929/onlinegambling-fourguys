...
# 1. 데이터 로드 및 캐싱 (파일명 확인 필요)
@st.cache_data
def load_data():
    # 데이터 불러오기 (환경에 맞게 경로를 조정하세요)
    df = pd.read_csv("onlineCasino.csv")
    # 문자열로 되어 있는 시간 데이터를 datetime 객체로 변환
...
with tab1:
    ...
    fig_heatmap = px.imshow(
        ...
        color_continuous_scale="RdBu_r", # 상관관계용 표준 컬러맵 (빨강/파랑)
        zmin=-1, zmax=1,                 # 상관계수 범위 고정
...
# --- ✨ 2. 산점도 (Scatter Plot) ---
with tab2:
    st.subheader("참여 게이머 수 vs 총 판돈(Money) 산点도")
    st.markdown("> 💡 **Tip:** 데이터가 5만 건 이상으로 너무 많아, 웹 브라우저의 렉을 방지하고 시각화 직관성을 높이기 위해 **무작위로 2,000건을 샘플링**하여 출력합니다.")
    
    # 대용량 데이터 시각화 최적화를 위한 램덤 샘플링
    ...
    fig_scatter = px.scatter(
        ...
        x="gamers",
        y="money",
        color="moderator",               # 중재자(True/False) 유무에 따른 색상 구분
        hover_data=["ticks", "outpay"],  # 마우스를 올렸을 때 추가로 보여줄 정보
        title="게이머 수와 판돈의 관계 (중재자 여부별 색상 구분)",
        labels={"gamers": "참여 게이머 수", "money": "총 판돈 (Money)"}
    )
    st.plotly_chart(fig_scatter, use_container_width=True)

# --- 📊 3. 분포 차트 (Distribution Chart / Histogram) ---
with tab3:
    st.subheader("배당률(Ticks) 분포 차트")
    st.markdown("현재 데이터의 배당률(`ticks`)은 최대 23,000이 넘는 극단적인 이상치가 존재합니다. 슬라이더를 조절하여 원하는 분포 범위를 확인해 보세요.")
    
    # 전처리 전 원시 데이터 처리를 위한 동적 슬라이더 구현
...
with tab4:
    ...
    # 이상치 유무에 따라 그래프가 찌그러지는 현상을 방지하기 위한 체크박스
    show_all = st.checkbox("아웃라이어(이상치)를 포함한 전체 데이터로 보기", value=False)
    
    if not show_all:
        # 상위 5%의 극단치 데이터를 제외하여 메인 분포가 잘 보이도록 설정
